...
async def test__upgrade_apps_in_bundle(ops_test):
    ...
    temp_dir = f"{tempfile.mkdtemp()}/kubeflow-bundle"

    res = subprocess.run(["charm", "pull", "cs:kubeflow", "--channel", "edge", temp_dir])

    with open(f"{temp_dir}/bundle.yaml") as bundle_file:
